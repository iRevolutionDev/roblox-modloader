...
namespace memory::rtti::utils
{
	namespace config
	{
		...
		constexpr std::array PROBLEMATIC_PATTERNS = {std::string_view{"<lambda_"}, std::string_view{"_Func_impl_no_alloc"}, std::string_view{"clone_impl"}, std::string_view{"sp_counted_impl"}, std::string_view{"shared_state"}};
	}
