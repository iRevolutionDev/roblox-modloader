...
public:
	...
	DenseHashTable(const Key &empty_key, size_t buckets = 0) : data(buckets, Item(empty_key)), count(0),
	                                                           empty_key(empty_key) {
	}
	...
	public:
		...
		const_iterator(const DenseHashTable *set, const size_t index) : set(set), index(index) {
		}
