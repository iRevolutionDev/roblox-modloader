...
void Math::vector_angles(const Vector3& forward, Vector3& angles)
{
	...
	{
		angles.x = atan2(-forward.z, forward.length()) * 180.0f / pi();
		angles.y = atan2(forward.y, forward.x) * 180.0f / pi();
...
//
Vector3 Math::degress_to_radians(Vector3 angles)
{
	angles.x = angles.x * pi() / 180.0f;
	angles.y = angles.y * pi() / 180.0f;
	angles.z = angles.z * pi() / 180.0f;
}

float Math::degrees_to_radians(float angle)
{
	return angle * pi() / 180.0f;
}
//
void Math::rotate_triangle(std::array<Vector2, 3>& points, float rotation)
{
