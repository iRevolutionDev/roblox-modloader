...
namespace rml::jobs
{
	...
	{
	public:
		LambdaJob(const std::string_view name, const JobPriority priority, const JobKind target_kind, std::function<bool(const JobExecutionContext&)> should_execute_func, std::function<void(const JobExecutionContext&)> execute_func) noexcept :
	...
	{
	public:
		PeriodicJob(const std::string_view name, const std::chrono::milliseconds interval, const JobPriority priority, const JobKind target_kind, std::function<void(const JobExecutionContext&)> execute_func) noexcept :
		    JobBase(name, priority, target_kind, true),
		    m_interval(interval),
		    m_execute_func(std::move(execute_func)),
