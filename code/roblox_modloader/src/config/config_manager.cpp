...
namespace rml::config
{
	ConfigResult<void> ConfigManager::load_config(const std::filesystem::path& config_path)
	{
		...
		try
		{
			const filesystem::File file(config_path);
			if (!file.exists())
			{
				return std::unexpected(ConfigError::file_not_found);
			}

			const auto table = file.read_toml();
			if (!table)
			{
				RML_ERROR("Failed to read configuration file '{}': {}", config_path.string(), table.error());
				return std::unexpected(ConfigError::parse_error);
			}

			return load_core_config(*table);
		}
	...
	ConfigResult<void> ConfigManager::load_or_create(const std::filesystem::path& config_path)
	{
		if (filesystem::File(config_path).exists())
		{
		...
		return write_defaults(config_path);
	}
	...
	ConfigResult<void> ConfigManager::save_config(const std::filesystem::path& config_path) const
	{
		...
		try
		{
			...
			serialized << root_table;

			if (const auto result = filesystem::File(config_path).write_text(serialized.str()); !result)
			{
				RML_ERROR("Failed to write configuration file '{}': {}", config_path.string(), result.error().message());
				return std::unexpected(ConfigError::io_error);
			}
	...
	void ConfigManager::watch_config_file(const std::filesystem::path& config_path, std::function<void()> callback)
	{
		m_watcher.start(config_path, std::chrono::milliseconds(1000), [this, config_path, callback = std::move(callback)] {
			if (const auto result = load_config(config_path); !result)
			{
	...
	void ConfigManager::backup_broken_config(const std::filesystem::path& config_path)
	{
		auto backup_path = config_path;
		backup_path += ".bak";

		if (const auto result = filesystem::File(config_path).move_to(backup_path); !result)
		{
			RML_WARN("Could not move the unusable configuration aside: {}", result.error().message());
		}
		else
		{
			RML_WARN("Moved the unusable configuration to '{}'", backup_path.string());
		}
	}

	ConfigResult<void> ConfigManager::write_defaults(const std::filesystem::path& config_path)
	{
		{
			std::unique_lock lock(m_config_mutex);
			m_core_config = CoreConfig{};
		}

		if (const auto result = save_config(config_path); !result)
		{
			RML_WARN("Continuing with built-in defaults, '{}' could not be written", config_path.string());
		}
