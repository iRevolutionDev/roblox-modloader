...
{
	bool Parser::parse(std::shared_ptr<ProcessInfo> info)
	{
		try
		{
	...
	std::shared_ptr<ProcessInfo> Parser::set_process_info(std::shared_ptr<ProcessInfo> info)
	{
		if (info)
		{
		...
		const auto new_info = std::make_shared<ProcessInfo>();
	...
	bool Parser::parse_sections()
	{
		...
		for (std::uint16_t i = 0; i < nt_headers->FileHeader.NumberOfSections; ++i)
		{
			...
			LOG_DEBUG("Parsed section: {} (VA: 0x{:X}, Size: 0x{:X})", section_name, virtual_address, virtual_size);
		}
