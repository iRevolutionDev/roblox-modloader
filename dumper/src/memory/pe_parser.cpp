#include "dumper/memory/pe_parser.hpp"
...
namespace memory::pe
{
	bool parser::parse(std::shared_ptr<process_info> info)
	{
		try
		{
			...
			if (!m_process_info)
			{
				spdlog::error("Failed to get process information");
				return false;
			}
		...
		{
			spdlog::error("PE parsing failed: {}", e.what());
			return false;
		}
	}
	...
	std::shared_ptr<process_info> parser::set_process_info(std::shared_ptr<process_info> info)
	{
		...
		if (!::GetModuleInformation(new_info->process_handle, new_info->process_module, new_info->module_info.get(), sizeof(MODULEINFO)))
		{
			spdlog::error("Failed to get module information: {}", ::GetLastError());
			return nullptr;
		}
	...
	bool parser::parse_sections()
	{
		...
		if (!base)
		{
			spdlog::error("Invalid module base address");
			return false;
		}
		...
		{
			spdlog::error("Invalid PE header offset: 0x{:X}", dos_header->e_lfanew);
			return false;
		}
		...
		{
			spdlog::error("Unreasonable number of sections: {}", nt_headers->FileHeader.NumberOfSections);
			return false;
		}

		const auto section_headers = reinterpret_cast<const IMAGE_SECTION_HEADER*>(
		    reinterpret_cast<const std::uint8_t*>(&nt_headers->OptionalHeader) + nt_headers->FileHeader.SizeOfOptionalHeader);
		...
		for (std::uint16_t i = 0; i < nt_headers->FileHeader.NumberOfSections; ++i)
		{
			...
			{
				spdlog::warn("Section {} too large ({} bytes), skipping", section_name, virtual_size);
				continue;
			}
			...
			auto section_info = std::make_unique<section>(section_name, virtual_size, start_offset, end_offset);

			m_section_map[section_name].emplace_back(std::move(section_info));

			spdlog::debug("Parsed section: {} (VA: 0x{:X}, Size: 0x{:X})", section_name, virtual_address, virtual_size);
		}

		spdlog::info("Successfully parsed {} sections", m_section_map.size());
		return true;
	}
}
