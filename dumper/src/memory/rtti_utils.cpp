...
namespace memory::rtti::utils
{
	...
	{
		return std::ranges::any_of(config::PROBLEMATIC_PATTERNS, [symbol](const std::string_view pattern) {
	...
	{
		return symbol.length() > config::MAX_SYMBOL_LENGTH || calculate_template_depth(symbol) > config::MAX_TEMPLATE_DEPTH || std::ranges::count(symbol, '@') > config::MAX_AT_SYMBOLS;
	}
