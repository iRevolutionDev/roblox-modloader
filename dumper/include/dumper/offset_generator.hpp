...
namespace dumper
{
	...
	private:
		...
		bool generate_json(const std::map<std::string, StructInfo>& structures, const std::filesystem::path& output_path);

		bool generate_genny(const std::map<std::string, StructInfo>& structures, const std::filesystem::path& output_path);
