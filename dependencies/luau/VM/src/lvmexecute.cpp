...
#endif
        {
            ...
            VM_CASE(LOP_SETTABLE) {
                ...
                if (ttistable(rb) && ttisnumber(rc)) {
                    ...
                    // index has to be an exact integer and in-bounds for the array portion
                    if (LUAU_LIKELY(
                        unsigned(index) - 1 < unsigned(h->sizearray) && !h->metatable && !h->readonly && double(index)
                        == indexd)) {
            ...
            VM_CASE(LOP_NAMECALL) {
                ...
                if (LUAU_LIKELY(ttistable(rb))) {
                    ...
                    // fast-path: key is absent from the base, table has an __index table, and it has the result in the expected slot
                             ...
                             (mtn = &hvalue(mt)->node[LUAU_INSN_C(insn) & hvalue(mt)->nodemask8]) && ttisstring(
                                 gkey(mtn)) &&
                ...
                if (ttype(ra) == ttype(rb)) {
                    switch (ttype(ra)) {
                        ...
                        case LUA_TLIGHTUSERDATA:
                            pc += (pvalue(ra) == pvalue(rb) && lightuserdatatag(ra) == lightuserdatatag(rb))
                                      ? LUAU_INSN_D(insn)
                                      : 1;
                            LUAU_ASSERT(unsigned(pc - cl->l.p->code) < unsigned(cl->l.p->sizecode));
                            VM_NEXT();
            ...
            VM_CASE(LOP_ADD) {
                ...
                if (LUAU_LIKELY(ttisnumber(rb) && ttisnumber(rc))) {
                    ...
                    const TValue *fn = 0;
                    if (ttisuserdata(rb) && (fn = luaT_gettmbyobj(L, rb, TM_ADD)) && ttisfunction(fn) && clvalue(fn)->
                        isC) {
                        // note: it's safe to push arguments past top for complicated reasons (see top of the file)
                        LUAU_ASSERT(L->top + 3 < L->stack + L->stacksize);
                        StkId top = L->top;
            ...
            VM_CASE(LOP_SUB) {
                ...
                if (LUAU_LIKELY(ttisnumber(rb) && ttisnumber(rc))) {
                    ...
                    // fast-path for userdata with C functions
                    const TValue *fn = 0;
                    if (ttisuserdata(rb) && (fn = luaT_gettmbyobj(L, rb, TM_SUB)) && ttisfunction(fn) && clvalue(fn)->
                        isC) {
                        // note: it's safe to push arguments past top for complicated reasons (see top of the file)
                        LUAU_ASSERT(L->top + 3 < L->stack + L->stacksize);
                        StkId top = L->top;
            ...
            VM_CASE(LOP_MUL) {
                ...
                if (LUAU_LIKELY(ttisnumber(rb) && ttisnumber(rc))) {
                    ...
                    const TValue *fn = 0;
                    if (ttisuserdata(rbc) && (fn = luaT_gettmbyobj(L, rbc, TM_MUL)) && ttisfunction(fn) && clvalue(fn)->
                        isC) {
                        // note: it's safe to push arguments past top for complicated reasons (see top of the file)
                        LUAU_ASSERT(L->top + 3 < L->stack + L->stacksize);
                        StkId top = L->top;
            ...
            VM_CASE(LOP_DIV) {
                ...
                if (LUAU_LIKELY(ttisnumber(rb) && ttisnumber(rc))) {
                    ...
                    StkId rbc = ttisnumber(rb) ? rc : rb;
                    const TValue *fn = 0;
                    if (ttisuserdata(rbc) && (fn = luaT_gettmbyobj(L, rbc, TM_DIV)) && ttisfunction(fn) && clvalue(fn)->
                        isC) {
                        // note: it's safe to push arguments past top for complicated reasons (see top of the file)
                        LUAU_ASSERT(L->top + 3 < L->stack + L->stacksize);
                        StkId top = L->top;
            ...
            VM_CASE(LOP_IDIV) {
                ...
                if (LUAU_LIKELY(ttisnumber(rb) && ttisnumber(rc))) {
                    ...
                    StkId rbc = ttisnumber(rb) ? rc : rb;
                    const TValue *fn = 0;
                    if (ttisuserdata(rbc) && (fn = luaT_gettmbyobj(L, rbc, TM_IDIV)) && ttisfunction(fn) && clvalue(fn)
                        ->isC) {
                        // note: it's safe to push arguments past top for complicated reasons (see top of the file)
                        LUAU_ASSERT(L->top + 3 < L->stack + L->stacksize);
                        StkId top = L->top;
            ...
            VM_CASE(LOP_MULK) {
                ...
                if (LUAU_LIKELY(ttisnumber(rb))) {
                    ...
                    // fast-path for userdata with C functions
                    const TValue *fn = 0;
                    if (ttisuserdata(rb) && (fn = luaT_gettmbyobj(L, rb, TM_MUL)) && ttisfunction(fn) && clvalue(fn)->
                        isC) {
                        // note: it's safe to push arguments past top for complicated reasons (see top of the file)
                        LUAU_ASSERT(L->top + 3 < L->stack + L->stacksize);
                        StkId top = L->top;
            ...
            VM_CASE(LOP_DIVK) {
                ...
                if (LUAU_LIKELY(ttisnumber(rb))) {
                    ...
                    // fast-path for userdata with C functions
                    const TValue *fn = 0;
                    if (ttisuserdata(rb) && (fn = luaT_gettmbyobj(L, rb, TM_DIV)) && ttisfunction(fn) && clvalue(fn)->
                        isC) {
                        // note: it's safe to push arguments past top for complicated reasons (see top of the file)
                        LUAU_ASSERT(L->top + 3 < L->stack + L->stacksize);
                        StkId top = L->top;
            ...
            VM_CASE(LOP_IDIVK) {
                ...
                if (LUAU_LIKELY(ttisnumber(rb))) {
                    ...
                    // fast-path for userdata with C functions
                    const TValue *fn = 0;
                    if (ttisuserdata(rb) && (fn = luaT_gettmbyobj(L, rb, TM_IDIV)) && ttisfunction(fn) && clvalue(fn)->
                        isC) {
                        // note: it's safe to push arguments past top for complicated reasons (see top of the file)
                        LUAU_ASSERT(L->top + 3 < L->stack + L->stacksize);
                        StkId top = L->top;
            ...
            VM_CASE(LOP_MINUS) {
                ...
                if (LUAU_LIKELY(ttisnumber(rb))) {
                    ...
                    // fast-path for userdata with C functions
                    const TValue *fn = 0;
                    if (ttisuserdata(rb) && (fn = luaT_gettmbyobj(L, rb, TM_UNM)) && ttisfunction(fn) && clvalue(fn)->
                        isC) {
                        // note: it's safe to push arguments past top for complicated reasons (see top of the file)
            ...
            VM_CASE(LOP_SETLIST) {
                ...
                if (!ttistable(ra))
                    return;
                // temporary workaround to weaken a rather powerful exploitation primitive in case of a MITM attack on bytecode
            ...
            VM_CASE(LOP_FORGPREP) {
                ...
                if (ttisfunction(ra)) {
                    ...
                    LuaTable *mt = ttistable(ra)
                                       ? hvalue(ra)->metatable
                                       : ttisuserdata(ra)
                                             ? uvalue(ra)->metatable
                                             : cast_to(LuaTable*, NULL);
...
void luau_execute(lua_State *L) {
    if (get_roblox_pointers()->luau_execute != nullptr)
        return get_roblox_pointers()->luau_execute(L);
