...
int luaD_rawrunprotected(lua_State *L, Pfunc f, void *ud) {
    if (get_roblox_pointers()->luaD_rawrunprotected != nullptr)
        return get_roblox_pointers()->luaD_rawrunprotected(L, f, ud);
...
l_noret luaD_throw(lua_State *L, int errcode) {
    if (get_roblox_pointers()->luaD_throw != nullptr) {
        return get_roblox_pointers()->luaD_throw(L, errcode);
    }
