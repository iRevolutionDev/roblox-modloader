...
#include "RobloxModLoader/hooking/call_hook.hpp"
...
call_hook_memory::call_hook_memory() {
	m_memory = VirtualAlloc((void *) ((uintptr_t) GetModuleHandle(0) + 0x20000000), 1024, MEM_COMMIT | MEM_RESERVE,
...
call_hook::call_hook(void *location, void *hook) : m_location(location),
                                                   m_hook(hook) {
